...
public class SkyScrapper {
	...
	public void calculateAndPrintOrderOfConstruction(Integer[] totalFloorSize) {
		...
		// 1. Loop over the unsorted data and when you see the entry is matched then pop
		// all Stack elements
		for (int i = 0; i < totalFloorSize.length; i++) {
			if (totalFloorSize[i] == totalFloorSizeSorted[j]) {
				...
				// Print the current floor as well as entire Stack till it's empty and entry is
				// > than next element in sorted list.
